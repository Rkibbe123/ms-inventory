...
def generate_cli_device_login_script(output_dir, tenant, subscription):
    ...
    script_parts.extend([
        ...
        "        ",
        "        # Get access tokens with tenant specification",
        "        Write-Host 'Getting access tokens from Azure CLI...' -ForegroundColor Yellow",
        "        ",
        "        # Get Management token (for Azure Resource Manager)",
        "        $managementToken = az account get-access-token --tenant $azContext.tenantId --resource https://management.azure.com --query accessToken --output tsv 2>$null",
        "        # Get Graph token (for Microsoft Graph API)",
        "        $graphToken = az account get-access-token --tenant $azContext.tenantId --resource https://graph.microsoft.com --query accessToken --output tsv 2>$null",
        "        ",
        "        if (-not $managementToken) {",
        "            Write-Host 'Failed to get management token, trying generic token...' -ForegroundColor Yellow",
        "            $managementToken = az account get-access-token --tenant $azContext.tenantId --query accessToken --output tsv 2>$null",
        "        }",
        "        ",
        "        if (-not $managementToken) {",
        ...
        "        ",
        "        # Convert tokens to SecureString for PowerShell",
        "        $secureManagementToken = ConvertTo-SecureString -String $managementToken -AsPlainText -Force",
        "        if ($graphToken) {",
        "            $secureGraphToken = ConvertTo-SecureString -String $graphToken -AsPlainText -Force",
        "        }",
        "        ",
        "        # Try multiple connection methods with proper token format",
        "        Write-Host 'Attempting PowerShell connection (Method 1: With both tokens)...' -ForegroundColor Yellow",
        "        try {",
        "            if ($graphToken) {",
        "                Connect-AzAccount -AccessToken $secureManagementToken -GraphAccessToken $secureGraphToken -AccountId $azContext.user.name -TenantId $azContext.tenantId -ErrorAction Stop",
        "                Write-Host 'Connected to Azure successfully with full token set!' -ForegroundColor Green",
        "            } else {",
        "                Connect-AzAccount -AccessToken $secureManagementToken -AccountId $azContext.user.name -TenantId $azContext.tenantId -ErrorAction Stop",
        "                Write-Host 'Connected to Azure successfully with management token!' -ForegroundColor Green",
        "            }",
        "        } catch {",
        "            Write-Host \"Method 1 failed: $($_.Exception.Message)\" -ForegroundColor Yellow",
        "            Write-Host 'Attempting PowerShell connection (Method 2: Simplified with token)...' -ForegroundColor Yellow",
        "            try {",
        "                # Try without AccountId in case that's causing issues",
        "                if ($graphToken) {",
        "                    Connect-AzAccount -AccessToken $secureManagementToken -GraphAccessToken $secureGraphToken -TenantId $azContext.tenantId -ErrorAction Stop",
        "                } else {",
        "                    Connect-AzAccount -AccessToken $secureManagementToken -TenantId $azContext.tenantId -ErrorAction Stop",
        "                }",
        ...
        "}",
        "",
        "# Verify the connection worked by testing basic cmdlet",
        "Write-Host 'Verifying Azure connection...' -ForegroundColor Yellow",
        "try {",
        "    $testContext = Get-AzContext -ErrorAction Stop",
        "    if ($null -eq $testContext) {",
        "        Write-Error 'Azure context is null after connection attempt'",
        "        exit 1",
        "    }",
        "    Write-Host \"✅ Connection verified!\" -ForegroundColor Green",
        "    Write-Host \"   Tenant: $($testContext.Tenant.Id)\" -ForegroundColor Cyan",
        "    Write-Host \"   Subscription: $($testContext.Subscription.Name) ($($testContext.Subscription.Id))\" -ForegroundColor Cyan",
        "    Write-Host \"   Account: $($testContext.Account.Id)\" -ForegroundColor Cyan",
        "} catch {",
        "    Write-Error \"Connection verification failed: $($_.Exception.Message)\"",
        "    Write-Host 'Attempting to re-authenticate...' -ForegroundColor Yellow",
        "    try {",
        "        # Try one more time with device authentication as absolute fallback",
        "        $cliContext = az account show --output json | ConvertFrom-Json",
        "        Connect-AzAccount -UseDeviceAuthentication -TenantId $cliContext.tenantId -ErrorAction Stop",
        "        $retryContext = Get-AzContext -ErrorAction Stop",
        "        if ($null -eq $retryContext) {",
        "            Write-Error 'Re-authentication failed - context still null'",
        "            exit 1",
        "        }",
        "        Write-Host '✅ Re-authentication successful!' -ForegroundColor Green",
        "    } catch {",
        "        Write-Error \"All authentication attempts exhausted: $($_.Exception.Message)\"",
        "        exit 1",
        "    }",
        "}"
    ])
