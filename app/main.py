...
def generate_cli_device_login_script(output_dir, tenant, subscription):
    ...
    script_parts.extend([
        ...
        "    $paramString = \"-ReportDir '$reportDir' -ReportName '$reportName' -TenantID '$tenantId' -SubscriptionID '$subscriptionId' -SkipDiagram -Debug -NoAutoUpdate\"",
        ...
        "    ",
        "    # Method 2: Try with direct cmdlet call",
        "    Write-Host 'Attempting ARI execution (Method 2: Direct Call)...' -ForegroundColor Yellow",
        "    try {",
        "        Invoke-ARI -ReportDir $reportDir -ReportName $reportName -TenantID $tenantId -SubscriptionID $subscriptionId -SkipDiagram -Debug -NoAutoUpdate",
        "        Write-Host 'Method 2 (Direct Call) completed successfully!' -ForegroundColor Green",
        "    } catch {",
        "        Write-Error \"All execution methods failed. Last error: $($_.Exception.Message)\"",
        "        exit 1",
        "    }",
        "}",
        "",
